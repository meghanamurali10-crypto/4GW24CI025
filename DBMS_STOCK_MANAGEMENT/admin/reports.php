...
if (($_GET['export'] ?? '') !== '') {
    ...
    if ($report === 'requests') {
        $headers = ['Request No','Department','Status','Purpose','Created'];
        foreach (rows('SELECT r.*, d.name department_name FROM requests r JOIN departments d ON d.id=r.department_id') as $r) {
            $data[] = [$r['request_no'],$r['department_name'],$r['status'],$r['purpose'],$r['created_at']];
        }
    }
    ...
    log_activity('Exported ' . $report . ' CSV report');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        foreach ($dataRows as $row) {
            ...
            $pdo->prepare('INSERT INTO items (item_code,item_name,category_id,quantity,unit,unit_price,minimum_stock,storage_location,description,created_by) VALUES (?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE item_name=VALUES(item_name), category_id=VALUES(category_id), unit=VALUES(unit), unit_price=VALUES(unit_price), minimum_stock=VALUES(minimum_stock), storage_location=VALUES(storage_location), description=VALUES(description)')
                ->execute([$code,$name,$cat['id'],(float)$qty,$unit ?: 'Nos',(float)$price,(float)$min,$location,$description,current_user()['id']]);
            $count++;
        }
        $pdo->prepare('INSERT INTO excel_import_logs (user_id, file_name, action_type, status, rows_processed, remarks) VALUES (?, ?, "IMPORT", "SUCCESS", ?, ?)')->execute([current_user()['id'], basename($path), $count, 'Imported through built-in CSV/XLSX handler.']);
        log_activity('Imported Excel/CSV file ' . basename($path));
...
?>
<h1 class="h3 mb-3">Reports, Excel Import & PDF Export</h1>
<div class="row g-3">
  <div class="col-lg-4"><div class="card metric-card"><div class="card-header bg-white fw-semibold">Excel Import</div><div class="card-body"><form method="post" enctype="multipart/form-data"><?php csrf_field(); ?><label class="form-label">Upload .csv or .xlsx</label><input name="excel" type="file" accept=".csv,.xlsx" class="form-control mb-3" required><button class="btn btn-primary">Import Items</button></form><div class="small text-muted mt-3">Template columns: code, name, category, quantity, unit, price, min, location, description.</div></div></div></div>
  <div class="col-lg-4"><div class="card metric-card"><div class="card-header bg-white fw-semibold">Excel Export</div><div class="card-body d-grid gap-2"><a class="btn btn-outline-primary" href="?export=stock">Export Stock</a><a class="btn btn-outline-primary" href="?export=categories">Export Categories</a><a class="btn btn-outline-primary" href="?export=requests">Export Requests</a></div></div></div>
  <div class="col-lg-4">
      ...
      <div class="card-body">
        ...
        <form method="get" action="<?= BASE_URL ?>/download_pdf.php" class="border-top pt-3">
          ...
          <select name="type" class="form-select mb-2">
            ...
            <option value="import_logs">Excel Import/Export Log PDF</option>
...
<div class="card metric-card mt-3"><div class="card-header bg-white fw-semibold">Import / Export Logs</div><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Time</th><th>User</th><th>File</th><th>Action</th><th>Status</th><th>Rows</th><th>Remarks</th></tr></thead><tbody><?php foreach($imports as $i): ?><tr><td><?= e($i['created_at']) ?></td><td><?= e($i['name']) ?></td><td><?= e($i['file_name']) ?></td><td><?= e($i['action_type']) ?></td><td><?= e($i['status']) ?></td><td><?= e($i['rows_processed']) ?></td><td><?= e($i['remarks']) ?></td></tr><?php endforeach; ?></tbody></table></div></div>
<?php render_footer(); ?>
